...
// This eliminates the need for agents to read spec.yaml, plan.yaml, and tasks.yaml
...
// bundled in a phase context file. Coding agents use this to avoid redundant
...
type ContextMeta struct {
	...
	// HasGovernance indicates whether project governance is bundled
	HasGovernance bool `yaml:"has_governance"`
	// GovernanceFile is the source path for bundled governance
	GovernanceFile string `yaml:"governance_file,omitempty"`
...
type PhaseContext struct {
	...
	// Governance contains the project constitution when one exists
...
# This file bundles spec, plan, governance, and phase-specific tasks for coding agents
...
// This allows coding agents to avoid redundant file reads.
func buildContextMeta(specDir string) ContextMeta {
	...
	if path, ok := constitution.FindFrom(specDir); ok {
		meta.HasGovernance = true
		meta.GovernanceFile = path
		meta.BundledArtifacts = append(meta.BundledArtifacts, filepath.Base(path))
		meta.SkipReads = append(meta.SkipReads, path)
	}
	return meta
}
...
func BuildPhaseContext(specDir string, phaseNumber int, totalPhases int) (*PhaseContext, error) {
	...
	if err := loadGovernanceIntoContext(ctx); err != nil {
		return nil, err
	}
...
// loadGovernanceIntoContext reads the constitution when one was discovered.
func loadGovernanceIntoContext(ctx *PhaseContext) error {
	path := ctx.ContextMeta.GovernanceFile
	if path == "" {
		return nil
	}
	...
	if err := yaml.Unmarshal(data, &ctx.Governance); err != nil {
		return fmt.Errorf("failed to parse constitution: %w", err)
	}
	return nil
}
