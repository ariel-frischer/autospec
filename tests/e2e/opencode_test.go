...
func TestE2E_OpenCodeAgentFlagPassesThrough(t *testing.T) {
	env := testutil.NewE2EEnvWithAgent(t, testutil.AgentOpencode)
	env.InitGitRepo()
	env.CreateBranch("001-test-feature")
	env.SetupConstitution()
	...
	result := env.Run("specify", "test feature", "--agent", "opencode", "--opencode-agent", "Build")
	require.Equal(t, 0, result.ExitCode, "specify failed\nstdout: %s\nstderr: %s",
		result.Stdout, result.Stderr)

	content, err := os.ReadFile(callLogPath)
	require.NoError(t, err, "mock call log should be written")
	log := string(content)
	require.Contains(t, log, `  - "--agent"`)
	require.Contains(t, log, `  - "Build"`)
}
